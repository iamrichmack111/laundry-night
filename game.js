...
function renderDaily(){
  ...
  state.daily.forEach((entry,i)=>{
    ...
    const el=document.createElement('article'); el.className='load-card'+(entry.done?' done':''); el.dataset.testid='load-card';
    el.innerHTML=`<div class="load-icon">${load.icon}</div><h3>${load.name}</h3><p>${load.desc}</p>
      ...
      <button class="${entry.done?'ghost-btn':'primary-btn'}" data-testid="begin-challenge" data-load="${load.id}" ${entry.done?'disabled':''}>${entry.done?'✓ COMPLETE':'BEGIN CHALLENGE'}</button>`;
